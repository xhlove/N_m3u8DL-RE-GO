...
// convertTwoToThree 转换2位到3位语言代码
func convertTwoToThree(input string) string {
	if code, exists := codeMap[input]; exists {
		return code
	}
	return input
}

// ConvertLangCodeAndDisplayName 转换ISO 639-1到ISO 639-2，并设置显示名称
func ConvertLangCodeAndDisplayName(langCode string, mediaType entity.MediaType, description string) (string, string) {
	if langCode == "" {
		return "und", description
	}
	...
	for _, lang := range allLanguages {
		if strings.EqualFold(lang.ExtendCode, langCode) ||
			strings.EqualFold(lang.Code, langCode) {
			foundLang = &lang
			break
		}
	}
	...
	if foundLang == nil {
		...
		if len(parts) > 0 {
			converted := convertTwoToThree(parts[0])
			for _, lang := range allLanguages {
				if strings.EqualFold(lang.ExtendCode, converted) ||
					strings.EqualFold(lang.Code, converted) {
					foundLang = &lang
					break
				}
			}
	...
	if foundLang != nil {
		...
		if description == "" {
			if mediaType == entity.MediaTypeSubtitles {
				description = foundLang.Description
			} else {
				description = foundLang.DescriptionAudio
			}
		}
	} else {
		langCode = "und" // 无法识别直接置为und
	}

	// 无描述，则把原始LangCode当作描述
	if description == "" {
		description = originalLangCode
	}

	return langCode, description
}

// FilterLanguage 过滤无效语言代码
func FilterLanguage(v string) string {
	if v == "" {
		return ""
	}

	// 检查是否为有效的语言代码格式
	langCodeRegex := regexp.MustCompile(`^[\w_\-\d]+$`)
	if langCodeRegex.MatchString(v) {
		return v
	}
	return "und"
}

// GetLanguageFromCode 从语言代码获取语言名称
func GetLanguageFromCode(code string) string {
	...
	for _, lang := range allLanguages {
		if strings.EqualFold(lang.ExtendCode, code) ||
			strings.EqualFold(lang.Code, code) {
...
// GetCodeFromLanguage 从语言名称获取语言代码
func GetCodeFromLanguage(language string) string {
	if language == "" {
		return ""
	}
	...
	for _, lang := range allLanguages {
		if strings.EqualFold(lang.Description, language) ||
			strings.EqualFold(lang.DescriptionAudio, language) {
			return lang.Code
		}
	}
